...
def is_server_ready(url, timeout=5):
    """Check if the server is connected and responds with 200"""
    try:
...
def gpu_worker(device_index, stop_event):
    ...
    print(f"[Worker-{device_index}] Received stop signal, ending task")
...
def main(server_url, max_wait=300):
    ...
    print(f"Performing stress test and detecting if server {server_url} is ready, maximum {max_wait} seconds...")

    # Detect while running stress tasks
    while time.time() - start_time < max_wait:
        if is_server_ready(server_url):
            print(f"✅ Detected server {server_url} is connected and responding")
            break
        time.sleep(1)
    ...
    if time.time() - start_time >= max_wait:
        print("⚠️ Timeout, service still not ready")
    else:
        print("GPU stress task completed, can proceed to next step")
